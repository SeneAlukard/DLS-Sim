...
#define LEADER_EVAL_INTERVAL 10 // Reevaluate leader every 60 seconds
...
void printDrones() {
  ...
  for (const auto &drone : drones) {
    std::cout << "  ID: " << drone.id << ", RSSI: " << drone.rssi
...
void statusCheckThread() {
  ...
  while (true) {
    ...
    for (auto &drone : dronesCopy) {
      try {
        ...
        if (spacePos != std::string::npos) {
          std::string status = response.substr(0, spacePos);
          std::string rssiStr = response.substr(spacePos + 1);

          if (status == "STATUS_OK") {
            int newRSSI = std::stoi(rssiStr);
            std::lock_guard<std::mutex> lock(droneMutex);
            for (auto &d : drones) {
              if (d.id == drone.id) {
                d.rssi = newRSSI; // Update RSSI level
                break;
              }
            }
          } else {
    ...
    // Reevaluate leader every LEADER_EVAL_INTERVAL seconds
    if (elapsedTime >= LEADER_EVAL_INTERVAL) {
      std::cout << "[DEBUG] Reevaluating leader based on RSSI levels.\n";
      selectLeader();
      elapsedTime = 0;
    }
  }
...
void handleNewConnection(int clientSocket) {
  ...
  if (bytesRead <= 0) {
    std::cerr << "[ERROR] Failed to read RSSI level from drone.\n";
    close(clientSocket);
    return;
  }

  int rssi = std::stoi(buffer);
  ...
  std::cout << "[INFO] Assigned ID " << newDrone.id << " to a drone with RSSI "
            << rssi << "\n";
